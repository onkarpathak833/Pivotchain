...
def updateAccountDetails(userId,txnDate,txnType,txnAmount):
    ...
    if cursor1.rowcount==0:
        ...
        for row in row_data:
            ...
            insert_stmt="UPDATE ACCOUNT_DETAILS SET TOTAL_BALANCE='%d',DAY_CLOSING_BALANCE='%d',NO_TXN='%d',AVG_TXN_AMOUNT='%f',LAST_TXN_DAY='%s',DAYS_OF_TXN='%d',MONTH_AVG_BALANCE='%f',TOTAL_TXN_VALUE='%d',DAYS_TXN_VALUE='%d' WHERE USER_ID='%s'"%(totalBalance,day_closing_balance,no_txn,avg_txn_amount,last_txn_day,days_of_txn,month_avg_balance,total_txn_value,txnAmount,userId)
...
def consumeMsgs():
    consumer = KafkaConsumer(KAFKA_TOPIC, bootstrap_servers=KAFKA_BROKERS, 
                         auto_offset_reset='earliest',client_id='client2',group_id='test-consumer-group')
